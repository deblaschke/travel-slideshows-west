// Park name to remove from description (initialized in slideshow.html)
// var parkNameToRemove = "National Park Name";

// getDescription returns description for compilation slide
    ...
    if (/^[0-9]{3}_/.test(file)) {
      ...
      if (result.length > 0) {
        ...
        if (index >= 0) {
          ...
          // Remove park name from description if defined
          if (typeof(parkNameToRemove) === 'string') {
            index = desc.indexOf("_in_" + parkNameToRemove);
            if (index >= 0) {
              desc = desc.substring(0, index);
            } else {
              index = desc.indexOf(parkNameToRemove);
              if (index >= 0) {
                desc = desc.substring(0, index);
              }
            }
