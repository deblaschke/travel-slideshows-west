// getDescription returns description for Yosemite compilation slide
    ...
    if (file.match(/^[0-9]{3}_/)) {
      ...
      if (result.length > 0) {
        ...
        if (index >= 0) {
          ...
          // Remove "*Yosemite National Park" from description
          index = desc.indexOf("_in_Yosemite_National_Park");
          if (index >= 0) {
            desc = desc.substring(0, index);
          } else {
            index = desc.indexOf("Yosemite_National_Park");
            if (index >= 0) {
              desc = desc.substring(0, index);
            }
          }
